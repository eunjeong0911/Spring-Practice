...
public class OAuth2Controller {
    ...
    public KakaoOAuthToken kakaoAccessToken(String code){
        //카카오 서버쪽으로 Post 방식 요청, Key-value
        ...
        params.add("code", code); //인증 직후 받은 code값 사용

        //header 와 body를 담은  HttpEntity 생성
        HttpEntity<MultiValueMap<String, String>> kakaoTokenRequest =
                new HttpEntity<>(params, headers);

        //요청
        ResponseEntity<String> response = rt.exchange(
                kakaoTokenUri,//Acess Token 발급 URI
                HttpMethod.POST, //request mwthod
                kakaoTokenRequest, //HttpEntity (body, header)
                String.class
        );
        System.out.println("카카오 AcessToken 요청 응답: " + response);
        //body만 확인해보면
    ...
    public User registerKakaoUser(KakaoProfile profile){
        //새로 가입시킹 username을 생성(unique 해야 한다!)
       ...
       """.formatted(username, name, password, provider, providerId));
        ...
        if(user == null){ //미가입자인 경우에만 회원 가입 진행
            User newUser = User.builder()
                    ...
                    .build();
            userService.register(newUser); //회원 가입 ! INSERT
            ...
            System.out.println("[kakao인증. 이미 가입된 회원입니다.]");
        }
        return user;
