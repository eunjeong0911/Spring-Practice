...
public class JpaBoardTest {
    ...
    @Autowired
    private PasswordEncoder passwordEncoder; //password 인코딩 주입받아서 써야함
    ...
    public void init(){
        ...
        admin1.addAuthority(auth_member, auth_admin);  //중간테이블에더 insert 됨

        userRepository.saveAll(List.of(user1, user2, admin1)); //저장
        userRepository.findAll().forEach(System.out::println); //확인

        //특정 User 권한 조회
        System.out.println("\nUser 및 권한 조회" + "-".repeat(20));
        user1 = userRepository.findById(1L).orElse(null); //Eager Fetch 걸어놓았기떄문에 User 읽어올떄 Authority 까지 불러온다.
